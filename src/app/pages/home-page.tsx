  ...
  return (
    <main className="min-h-screen bg-[#F7F7F7] px-6 py-10 text-[#222222] sm:px-10 lg:px-16">
      <section className="mx-auto max-w-6xl">
        <p className="text-sm font-semibold uppercase tracking-[0.2em] text-[#C93228]">
          Restaurant Platform
        </p>
        ...
        <p className="mt-4 max-w-2xl text-base leading-6 text-[#222222]/60">
          Explore the customer restaurant finder or open the restaurant CRM workspace for
          staff and managers.
        </p>

        <div className="mt-10 grid gap-4 md:grid-cols-3">
          {applicationSections.map(({ description, label, title, to }) => (
            <Link
              ...
              to={to}
            >
              <p className="text-sm font-semibold text-[#C93228]">{label}</p>

              <h2 className="mt-4 text-2xl font-semibold">{title}</h2>

              <p className="mt-3 text-sm leading-6 text-[#222222]/60">{description}</p>
