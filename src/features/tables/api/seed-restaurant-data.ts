...
/**
 * One-time Firestore seeder. Not used by the running app.
 * Import and call it from the browser console when you need demo data.
 */
export async function seedRestaurantData(): Promise<void> {
  const batch = writeBatch(db);

  tables.forEach((table) => {
    batch.set(doc(db, firestoreCollections.tables, table.id), {
      number: table.number,
      reservationTime: table.reservationTime,
      status: table.status,
    });

    const items = ordersByTableId[table.id] ?? [];

    batch.set(doc(db, firestoreCollections.tableOrders, table.id), {
      tableId: table.id,
